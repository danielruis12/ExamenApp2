...
namespace EXAMEN2.Clases
{
    public class clsUpload
    {
                    ...
                    foreach (MultipartFileData file in provider.FileData)
                    {
                        ...
                        if (File.Exists(Path.Combine(root, fileName)))
                        {
                            if (Actualizar)
                            {
                                ...
                                return request.CreateResponse(System.Net.HttpStatusCode.OK, "Se actualizó la Foto");
                            }
                            else
                            {
                                //El archivo ya existe en el servidor, no se va a cargar, se va a eliminar el temporal y se devolverá un error
        ...
        public HttpResponseMessage DescargarArchivo(string Foto)
        {
            try
            {
                string Ruta = HttpContext.Current.Server.MapPath("~/Archivos");
                string Archivo = Path.Combine(Ruta, Foto);
                if (File.Exists(Archivo))
                {
                    ...
                    response.Content.Headers.ContentDisposition.FileName = Foto;
        ...
        public HttpResponseMessage EliminarArchivo(string nombreArchivo)
        {
            try
            {
                ...
                if (File.Exists(archivoCompleto))
                {
                    ...
                    return request.CreateResponse(HttpStatusCode.OK, $"Archivo '{nombreArchivo}' eliminado correctamente.");
                }
                else
                {
                    return request.CreateErrorResponse(HttpStatusCode.NotFound, $"El archivo '{nombreArchivo}' no existe.");
                }
            }
            catch (Exception ex)
            {
